...
class DVMS:
    ...
    # Tracking: 38, Metadata: 40, Physical Splits: 42, Physical Summary: 43
    SUBTYPE_TRACKING = 38
    SUBTYPE_METADATA = 40
    SUBTYPE_SPLITS = 42
    SUBTYPE_SUMMARY = 43
    ...
    def fixtures(self, *, format: str = "dataframe"):
        ...
        # Build DF (normalized opta_match_id) -> keep as Polars internally
        df = (
    ...
    def _ensure_fixtures_loaded(self) -> None:
        """
        Lazy-prime caches so callers (splits/summary) don't need to call fixtures() explicitly.
        """
        if (
            self._fixtures_df is None
            or self._fixture_assets is None
            or self._opta_competition_id is None
        ):
            # Prime caches based on constructor params (season/competition/creds)
            self.fixtures(format="dataframe")
    ...
    def splits(self, *, opta_match_id: str, type: str = "players", model_form: str = "denormalized") -> pl.DataFrame:
        ...
        t = type.lower()
        mf = model_form.lower()
        if t == "players" and mf == "denormalized":
            return players_df
        if t == "players" and mf == "normalized":
            return players_df_normalized
        if t == "teams" and mf == "denormalized":
            return teams_df
        if t == "teams" and mf == "normalized":
            return teams_df_normalized
        raise ValueError("type must be 'players' or 'teams'")

    def summary(self, *, opta_match_id: str, model_form: str = "denormalized") -> pl.DataFrame:
        ...
        metadata_df = pl.from_dicts([metadata_raw])
        physical_summary_df = physical_summary(
            self._fixtures_df,          # type: ignore[arg-type]
            metadata_df,
            summary_csv,
            opta_match_id,
        )
        # (If you add a different normalized schema later, switch here)
        return physical_summary_df
    ...
    def _find_asset(self, *, opta_match_id: str, sub_type: int) -> dict:
        if not self._fixture_assets:
            # Shouldn't happen now (lazy primed), but keep friendly message:
            raise RuntimeError("No cached assets after fixtures() priming.")
